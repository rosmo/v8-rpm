...
Patch: 		v8-4.5-no-wshorten.patch

BuildRoot:      %{_tmppath}/%{name}-%{version}-%{release}-root-%(%{__id_u} -n)
BuildRequires:	python >= 2.4, python-argparse
...
# Snapshots seem to cause segfault on V8 >4.4
%ifarch x86_64
make x64.release library=shared soname_version=%lib_version snapshot=off %{?_smp_mflags}
%else
make ia32.release library=shared soname_version=%lib_version snapshot=off %{?_smp_mflags}
%endif
...
%changelog
* Mon Aug 17 2015 Taneli Leppa <taneli@crasman.fi> - 4.5.103.22-1
- bumped to upstream 4.5.103.22

* Thu Aug 13 2015 Taneli Leppa <taneli@crasman.fi> - 4.4.63.31-1
- bumped to upstream 4.4.63.31
